...
function DocChip({ nombre, tipo, url }) {
  ...
  const inner = (
    <div className="flex items-center gap-2.5 bg-white rounded-xl border border-gray-100 shadow-sm px-3 py-2.5">
      ...
      <svg className={`w-4 h-4 flex-shrink-0 ${url ? 'text-kof-red' : 'text-gray-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
      </svg>
    </div>
  )
  if (url) return <a href={url} target="_blank" rel="noreferrer" className="block active:opacity-70 transition-opacity">{inner}</a>
  return inner
}
